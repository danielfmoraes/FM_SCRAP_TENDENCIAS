def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Coleta e análise de tendências em Facility Management')
    
    parser.add_argument('--sources', nargs='+', choices=['ifma', 'google_scholar', 'all'],
...
def main(args):
    ...
    logger = logging.getLogger(__name__)
    logger.info("Iniciando análise de tendências em Facility Management")
    ...
    if args.sources:
        ...
        if args.report:
            report_path = generate_report(df, args.output)
...
if __name__ == "__main__":
    # Importações necessárias (certifique-se de que estão no início do arquivo)
    import os
    import sys
    import argparse
    import logging
    ...
    from src.scrapers import AbrafacScraper, InfraFMScraper, GoogleScholarScraper
    from src.processors import TextProcessor, translate_text, categorize_article
    from src.utils import setup_logging, ensure_dir, load_articles
